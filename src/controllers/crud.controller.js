...
function validateCollectionName(collection_name) {
  if (!VALID_COLLECTIONS.includes(collection_name)) {
    throw new Error(
      `Invalid collection name. Valid collections: ${VALID_COLLECTIONS.join(
        ", "
      )}`
    );
  }
...
export async function getAll(c) {
  try {
    ...
    const query_params = {
      ...
      include_relations: c.req.query("include_relations") === "true",
    };
...
export async function hardDelete(c) {
  try {
    const collection_name = c.req.param("collection_name");
    const id = c.req.param("id");
    ...
    const result = await CrudService.hardDelete(collection_name, id);
    return successResponse({ message: result.message });
  } catch (error) {
...
export async function flushDelete() {
  try {
    const collection = [
      "users",
      "positions",
    ...
    const result = await CrudService.flushDelete(collection);

    return successResponse({ message: result.message });
  } catch (error) {
    console.error("Error in flushDelete:", error);
    const status = error.message.includes("not found") ? 404 : 400;
    return errorResponse(error.message, status);
  }
}

export async function bulkCreate(c) {
  try {
    const collection_name = c.req.param("collection_name");
    validateCollectionName(collection_name);
    ...
    if (!data || !Array.isArray(data) || data.length === 0) {
      return errorResponse("Request body must be a non-empty array", 400);
    }

    const result = await CrudService.bulkCreate(collection_name, data);
    return successResponse(result);
  } catch (error) {
